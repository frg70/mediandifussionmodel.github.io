...
def cosine_beta_schedule(timesteps, s = 0.008): #function to be redefined with the new mapping of hybrid learning 
...
"""
#Nota: 
# 1. Hacer un experimento con cosine normal para probar reducción de tiempos 
# 2. Hacer experimento conjunto para probar que tal sirve el hybrid-learning
def cosine_beta_schedule(timesteps, s=0.008): 
    steps = timesteps +1 
    temp = []
    x = torch.linspace(0, timesteps, steps, dtype=torch.float64)
    alphas_cumprod_cos = torch.cos(((x/timesteps)+s)/ (1+s)*torch.pi*0.5)**2
    alphas_cumprod_cos = alphas_cumprod_cos/alphas_cumprod_cos[0]
    alphas_cumprod_senh =  torch.sinh((((x/timesteps) + s)/ (1.3 + 1.5*s))**0.5)
    alphas_cumprod_senh = torch.flip(alphas_cumprod_senh, [0])
    alphas_cumprod_senh = alphas_cumprod_senh/alphas_cumprod_senh[0]
    for i in range(len(alphas_cumprod_cos)): 
        if i%2 == 0 or i == 0: 
            temp.append(alphas_cumprod_cos[i])
        else: 
            temp.append(alphas_cumprod_senh[i])
    temp1 = torch.Tensor(temp)
    betas = 1 - (temp1[1:]/temp1[:-1])  
    return torch.clip(betas, 0, 0.9999)"""
class GaussianDiffusion(nn.Module): #to be redefined with the averages  
    def __init__(
        self,
        denoise_fn,
        *,
